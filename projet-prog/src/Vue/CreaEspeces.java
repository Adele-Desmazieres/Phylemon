...
public class CreaEspeces extends JPanel {
    ...
    public void creaCreaEspece(Controleur controleur, ChoixEspeces choixEspeces, Vue vue) {
        ...
        valider.addActionListener(event -> {
            if (profilComplet(nomJT.getText(), genomeJT.getText())) {
                int genome = Integer.parseInt(genomeJT.getText());
                boolean existeDeja = controleur.ajouterNouvEspece(nomJT.getText(),genome,descriptionJT.getText(),imageJT.getText());
                if (!existeDeja) JOptionPane.showMessageDialog(this,"Cette espèce existe déjà !"); //Si l'espèce n'as pas été ajouté, c'est qu'elle existe déja.
                else {
                ...
                // Pas de nom
                if (nomJT.getText().equals("")) JOptionPane.showMessageDialog(this,"Veuillez indiquez le nom de l'espèce.");
                // Pas de genome
                if (genomeJT.getText().equals("")) JOptionPane.showMessageDialog(this,"Veuillez indiquez le genome de l'espèce.");
                // genome incorrect
                if (!estNombre(genomeJT.getText())) JOptionPane.showMessageDialog(this,"Le génome de l'espèce est incorrect");
            }
        });
    ...
    private boolean profilComplet(String nom, String genome) {
        if (nom.equals("") || genome.equals("")) return false;
        return estNombre(genome);
    }
